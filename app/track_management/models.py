...
class mdlGPXTrack(models.Model):    
	...
	extensions = models.TextField("Extensions.", blank=True, null=True)	
	trackLine= models.MultiLineStringField()
...
class mdlGPXTrackSegment(models.Model):    
    gpx_track = models.ForeignKey(mdlGPXTrack)	
	extensions = models.TextField("Extensions.", blank=True, null=True)
	segmentLine= models.MultiLineStringField()
...
class mdlGPXTrackSegmentPoint(models.Model):
	...
	heart_rate = models.DecimalField("Cadence represents the cadence in revolutions per minute..", max_digits = 6,  decimal_places = 0 ,blank=True, null=True)
